...
VO_FILE = ROOT / "result" / "week5_6_strong_reset_debug_outdoors5" / "trajectory_tum.txt"
...
OUT = ROOT / "result" / "week8_10_outdoors5_ALL_FAST_IMU_NOISE"
OUT.mkdir(parents=True, exist_ok=True)

OUT_TRAJ_RAW = OUT / "outdoors5_vio_raw_tum.txt"
OUT_TRAJ_FILTERED = OUT / "outdoors5_vio_filtered_tum.txt"
OUT_METRICS = OUT / "outdoors5_metrics.txt"
OUT_XY = OUT / "outdoors5_xy.png"
OUT_XZ = OUT / "outdoors5_xz.png"


# ============================================================
# CONFIG: OUTDOOR5-SPECIFIC
# ============================================================
START_FRAME_INDEX = 0
END_FRAME_INDEX = None

# Outdoor5 is long; use coarse keyframes so runtime stays manageable.
KEYFRAME_STEP = 200
MAX_NFEV = 10
MAX_ASSOC_DT = 0.05

# Your Outdoor5 sync already worked well around 0, but allow small offsets.
TIME_OFFSETS = np.array([-0.02, -0.01, 0.0, 0.01, 0.02])

# Visual/scale priors. Outdoor5 has no reliable full GT; do not overfit endpoints.
SIGMA_VIS_POS = 0.08
SIGMA_SCALE_PRIOR = 0.45
SIGMA_ANCHOR = 1e-5
SIGMA_END_ANCHOR = 1e9       # disabled: endpoint is weak for Outdoor5
SIGMA_BA_PRIOR = 0.50
SIGMA_ROT_PRIOR = 0.12
MAX_ROT_CORR = 0.30

# Dataset IMU noise values are tiny. These practical floors prevent the simplified
# backend from over-trusting IMU residuals and exploding.
SIGMA_IMU_ROT_FLOOR = 0.05
SIGMA_IMU_VEL_FLOOR = 2.00
SIGMA_IMU_POS_FLOOR = 2.00

# Outdoor5 previous stable result used around 0.30 clipped scale.
# Keep broad enough to avoid killing the optimizer but bounded enough to avoid absurd scale.
SCALE_LOW = 0.05
SCALE_HIGH = 0.35
...
def load_imu_noise_config():
    if not IMU_CONFIG.exists():
        print("WARNING: imu_config.yaml not found. Using fallback TUM-VI-like IMU noise.")
        return {
...
def plot_results(est_p, gt_p, title):
    plt.figure(figsize=(8, 8))
    plt.plot(est_p[:, 0], est_p[:, 1], color="blue", label="VIO filtered", linewidth=1.5)
    plt.scatter(gt_p[0, 0], gt_p[0, 1], color="green", s=90, marker="o", label="GT start")
    plt.scatter(gt_p[-1, 0], gt_p[-1, 1], color="red", s=90, marker="X", label="GT end")
    plt.plot([gt_p[0, 0], gt_p[-1, 0]], [gt_p[0, 1], gt_p[-1, 1]], color="purple", linestyle="--", linewidth=2, label="GT start-end line")
    plt.axis("equal")
    plt.grid(True)
    plt.legend()
    plt.xlabel("x [m]")
    ...
    plt.figure(figsize=(8, 8))
    plt.plot(est_p[:, 0], est_p[:, 2], color="blue", label="VIO filtered", linewidth=1.5)
    plt.scatter(gt_p[0, 0], gt_p[0, 2], color="green", s=90, marker="o", label="GT start")
    plt.scatter(gt_p[-1, 0], gt_p[-1, 2], color="red", s=90, marker="X", label="GT end")
    plt.plot([gt_p[0, 0], gt_p[-1, 0]], [gt_p[0, 2], gt_p[-1, 2]], color="purple", linestyle="--", linewidth=2, label="GT start-end line")
    plt.axis("equal")
    plt.grid(True)
    plt.legend()
    plt.xlabel("x [m]")
...
# ============================================================
def run():
    print("=== OUTDOORS5 ALL-FRAMES FAST WEEK 8-10 VIO WITH IMU NOISE ===")
    print("Uses T_cam_imu and imu_config.yaml-based residual weighting.")
    ...
    print("IMU noise config:")
    for k, v in imu_noise.items():
        print(f"  {k}: {v}")
    print("IMU residual floors:")
    print(f"  sigma_rot_floor: {SIGMA_IMU_ROT_FLOOR}")
    print(f"  sigma_vel_floor: {SIGMA_IMU_VEL_FLOOR}")
    print(f"  sigma_pos_floor: {SIGMA_IMU_POS_FLOOR}")
    ...
    print(f"Using ALL segment: {START_FRAME_INDEX}:{end_idx}")
    ...
    print(f"scale abs   : {abs(best_init['init']['scale']):.6f}")
    ...
    gravity = 9.81 * best_init["init"]["gravity"] / (np.linalg.norm(best_init["init"]["gravity"]) + 1e-12)
    ...
    if opt is None:
        raise RuntimeError("Optimizer failed even after abs(scale).")
    ...
    print("\nFINAL OUTDOORS5 ALL-FRAMES FAST RESULT")
    print("=====================================")
    print(f"keyframes             : {len(kf_indices)}")
    print(f"time offset            : {best_init['td']:+.3f} s")
    print(f"scale raw abs init    : {opt['scale_init_raw_abs']:.6f}")
    print(f"scale clipped init    : {opt['scale_init_clipped']:.6f}")
    ...
    print(f"raw drift             : {raw_eval['drift']:.6f} m")
    print(f"raw drift percent     : {raw_eval['drift_percent']:.3f}%")
    print(f"filtered drift        : {filt_eval['drift']:.6f} m")
    print(f"filtered drift pct    : {filt_eval['drift_percent']:.3f}%")
    print(f"GT start-end length   : {filt_eval['gt_length']:.6f} m")
    print(f"start/end dt          : {filt_eval['start_dt']:.4f}s / {filt_eval['end_dt']:.4f}s")

    plot_results(dense_filtered, gt_p, f"Outdoor5 VIO IMU-noise | drift={filt_eval['drift']:.3f} m")

    with open(OUT_METRICS, "w", encoding="utf-8") as f:
        f.write("OUTDOORS5 ALL-FRAMES WEEK 8-10 VIO WITH DATASET IMU NOISE\n")
        f.write("========================================================\n")
        f.write("No full ATE/RPE is reported for Outdoor5; evaluation is start-end + qualitative.\n\n")
        f.write(f"segment: {START_FRAME_INDEX}:{end_idx}\n")
        f.write(f"keyframe_step: {KEYFRAME_STEP}\n")
        f.write(f"keyframes: {len(kf_indices)}\n")
        f.write(f"max_nfev: {MAX_NFEV}\n")
        f.write(f"time_offset: {best_init['td']}\n")
        f.write(f"gyro_bias: {best_init['bg'].tolist()}\n")
        f.write(f"gyro_score: {best_init['gyro_score']:.9f}\n")
        f.write(f"imu_noise_config: {imu_noise}\n")
        f.write(f"sigma_imu_rot_floor: {SIGMA_IMU_ROT_FLOOR}\n")
        f.write(f"sigma_imu_vel_floor: {SIGMA_IMU_VEL_FLOOR}\n")
        f.write(f"sigma_imu_pos_floor: {SIGMA_IMU_POS_FLOOR}\n")
        f.write(f"init_scale_raw: {best_init['init']['scale']:.9f}\n")
        f.write(f"init_scale_abs: {abs(best_init['init']['scale']):.9f}\n")
        ...
        f.write(f"cost0: {opt['cost0']:.9f}\n")
        f.write(f"cost1: {opt['cost1']:.9f}\n")
        f.write(f"raw_drift_m: {raw_eval['drift']:.9f}\n")
        f.write(f"raw_drift_percent: {raw_eval['drift_percent']:.9f}\n")
        f.write(f"filtered_drift_m: {filt_eval['drift']:.9f}\n")
        f.write(f"filtered_drift_percent: {filt_eval['drift_percent']:.9f}\n")
