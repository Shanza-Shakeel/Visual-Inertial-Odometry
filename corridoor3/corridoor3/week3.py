...
OUT = ROOT / "result" / "week3_4_improved_corridor4"
...
def load_ground_truth():
    ...
    with open(GT_CSV, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            # Not used in this script except storage
            qw = float(row[4])
            qx = float(row[5])
            qy = float(row[6])
            qz = float(row[7])
...
def save_tracking_image(img, p1, p2, frame_id):
    ...
    for x, y in zip(a, b):
        ...
        cv2.line(canvas, (x1, y1), (x2, y2), (0, 180, 255), 1)

    cv2.imwrite(str(OUT / f"tracking_{frame_id:04d}.png"), canvas)
...
def plot_trajectory(vo_traj, gt):
    ...
    plt.title("Corridor4 VO Trajectory vs Ground Truth")
...
def run():
    ...
    print("=== WEEK 3-4 CORRIDOR4 VO FRONTEND ===")
    ...
    for i in range(1, len(paths)):
        ...
        if tracked_curr is None or len(tracked_curr) < 100:
            ...
            stats.append((i, 0, 0, 0.0, 0.0, "RESET"))
            continue
