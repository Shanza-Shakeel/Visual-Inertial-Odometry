...
OUT = ROOT / "result" / "week5_6_strong_reset_debug"
...
# ================= FEATURES =================
def detect_grid(img, max_features=900):
    h, w = img.shape
...
# ================= PNP + BA + DIAGNOSTICS =================
def median_reproj_norm(Xw, obs_norm, R, t):
    ...
    proj = Xc[valid, :2] / Xc[valid, 2:3]
    err = np.linalg.norm(proj - obs_norm[valid], axis=1)
...
def reproj_diagnostics(Xw, pts_px, R, t, K, D):
    """
    Gives both normalized-coordinate reprojection error and pixel reprojection error.

    err_norm:
        Error after undistorting image observations into normalized camera coordinates.

    err_px:
        Error after projecting normalized prediction back through fisheye distortion into pixels.
    """
...
def save(ts_used, poses_cw, stats):
    ...
    plt.xlabel("x")
    plt.ylabel("z")
    plt.title("Week 5-6 Iterative PnP + BA VO + Strong Reset")
    plt.axis("equal")
    plt.grid(True)
...
def run():
    ...
    print("=== WEEK 5-6 ITERATIVE PNP + BA VO + STRONG RESET DEBUG ===")
