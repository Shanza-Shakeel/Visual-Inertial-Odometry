...
EST_FILE = ROOT / "result" / "week5_6_strong_reset_debug" / "trajectory_tum.txt"
GT_FILE = SEQ / "mav0" / "mocap0" / "data.csv"

OUT = ROOT / "result" / "week7_vo_eval"
...
def load_est(path):
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            v = line.split()
            ts = float(v[0])
            p = np.array(list(map(float, v[1:4])))
            data.append((ts, p))
    return data
...
def load_gt(path):
    ...
    with open(path, "r") as f:
        ...
        for row in r:
            ts = float(row[0]) * 1e-9
            p = np.array(list(map(float, row[1:4])))
            data.append((ts, p))
    return data


def associate(est, gt, max_dt=0.02):
    gt_ts = np.array([x[0] for x in gt])
    pairs = []

    for ts, p in est:
        idx = np.argmin(np.abs(gt_ts - ts))
        if abs(gt_ts[idx] - ts) <= max_dt:
            pairs.append((p, gt[idx][1], ts))

    if len(pairs) < 20:
        raise RuntimeError("Too few associated poses.")
...
def rpe_translation(aligned, gt, delta=10):
    errs = []
...
def main():
    ...
    rpe, rpe_err = rpe_translation(aligned, gt_p, delta=10)

    drift = np.linalg.norm((aligned[-1] - aligned[0]) - (gt_p[-1] - gt_p[0]))
    gt_len = path_length(gt_p)
    drift_pct = 100.0 * drift / gt_len

    print("\n=== WEEK 7 VO EVALUATION ===")
    ...
    with open(OUT / "metrics.txt", "w") as f:
        ...
        f.write(f"RPE RMSE delta=10 frames: {rpe:.4f} m\n")
        f.write(f"Start-end drift: {drift:.4f} m\n")
        f.write(f"Drift percentage: {drift_pct:.2f} %\n")
    ...
    plt.title("RPE translation, delta=10 frames")
